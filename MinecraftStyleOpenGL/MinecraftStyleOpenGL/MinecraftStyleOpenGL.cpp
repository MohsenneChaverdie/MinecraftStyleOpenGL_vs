...
struct MyApp : public Helper::App
{
	...
	void init()
	{
		...
		cubeSkyBox = new Helper::Cube(
			shader->id(),
			R"(D:\GitHub\uniCGL\MinecraftStyleOpenGL_cMake\OpenGLminecraftStyle\Textures\skybox.bmp)");

		cubeGrass = new Helper::Cube(
			shader->id(),
			R"(D:\GitHub\uniCGL\MinecraftStyleOpenGL_cMake\OpenGLminecraftStyle\Textures\grass.bmp)");

		cubeStone = new Helper::Cube(
			shader->id(),
			R"(D:\GitHub\uniCGL\MinecraftStyleOpenGL_cMake\OpenGLminecraftStyle\Textures\stone.bmp)");

		cubeDirt = new Helper::Cube(
			shader->id(),
			R"(D:\GitHub\uniCGL\MinecraftStyleOpenGL_cMake\OpenGLminecraftStyle\Textures\dirt.bmp)");

		cubeWater = new Helper::Cube(
			shader->id(),
			R"(D:\GitHub\uniCGL\MinecraftStyleOpenGL_cMake\OpenGLminecraftStyle\Textures\water.bmp)");

		cubeCloud = new Helper::Cube(
			shader->id(),
			R"(D:\GitHub\uniCGL\MinecraftStyleOpenGL_cMake\OpenGLminecraftStyle\Textures\cloud.bmp)");
		...
		int array_size_dirt_layer = (int)(height_multiplier + (float)total_Height);

		generated_rand_dirt_layer.reserve(array_size_dirt_layer);

		generated_rand_cloud_layer.reserve(30);

		// filling a vector to have on random some dirt layers
		for (int i = 0; i < array_size_dirt_layer; ++i)
		{
